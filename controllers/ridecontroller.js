...
module.exports.updatelocation = async (req, res) => {
    try {
        const { rideId } = req.params;
        const { sorce, destination, date, time, sorceLocation, destinationLocation } = req.body;
        // console.log(rideId);
        if (!mongoose.Types.ObjectId.isValid(rideId)) {
            return res.status(400).json({ success: false, message: "Invalid ride id" });
        }

        if (!String(sorce || "").trim() || !String(destination || "").trim() || !String(date || "").trim() || !String(time || "").trim()) {
            return res.status(400).json({ success: false, message: "sorce, destination, date and time are required" });
        }

        const rideDateTime = new Date(`${String(date).trim()}T${String(time).trim()}`);
        ...
        ride.date = rideDateTime;
        ride.time = String(time).trim();
        ride.sorceLocation = {
            type: "Point",
...
// GET /ridesynk/ride/:rideId/status
// Returns current ride status for client-side sync on page load
// Used by syncRideStatusFromServer() in map_page.js
